...
const PersistentSearchControl = require('./persistent-search-control')
const Control = require('../control')

tap.test('contructor', t => {
  t.test('new no args', async t => {
    const control = new PersistentSearchControl()
    t.ok(control)
    t.type(control, PersistentSearchControl)
    t.type(control, Control)
    t.equal(control.type, PersistentSearchControl.OID)
  ...
  t.test('with value buffer', async t => {
    ...
    const control = new PersistentSearchControl({ value: value.buffer })
    t.same(control.value, {
      changeTypes: 2,
      changesOnly: true,
      returnECs: false
  ...
  t.test('throws for bad value', async t => {
    t.throws(() => new PersistentSearchControl({ value: 42 }))
  })
...
tap.test('#toBer', t => {
  t.test('converts empty instance to BER', async t => {
    ...
    target.writeString(PersistentSearchControl.OID)
...
tap.test('#plainObject', t => {
  t.test('adds control value', async t => {
    ...
    t.same(value, {
      controlType: PersistentSearchControl.OID,
      criticality: false,
      controlValue: {
        changeTypes: 15,
        changesOnly: true,
